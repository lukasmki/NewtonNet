...
class BatchDataset(Dataset):
    ...
    def __init__(self, input, device):
        ...
        self.Z = torch.tensor(input["Z"], dtype=torch.long, device=device)
        self.E = torch.tensor(
            input["E"],
            # dtype=torch.float64,
            device=device,
        ...
        self.AM = torch.tensor(input["AM"], dtype=torch.long, device=device)
        if "NM" in input and input["NM"] is not None:
            self.N = torch.tensor(input["N"], dtype=torch.long, device=device)
            self.NM = torch.tensor(input["NM"], dtype=torch.long, device=device)

        # rotation matrix
        self.RM = None
        if "RM" in input and input["RM"] is not None:
            self.RM = torch.tensor(
                input["RM"],
                # dtype=torch.float64,
                device=device,
            )
...
def batch_dataset_converter(input, device):
    ...
    if "E" in input:
        result["E"] = torch.tensor(input["E"], dtype=torch.float32, device=device)
    if "F" in input:
        result["F"] = torch.tensor(input["F"], dtype=torch.float32, device=device)
    else:
        result["F"] = None
    if "CS" in input:
        result["CS"] = torch.tensor(input["CS"], dtype=torch.float32, device=device)
    if "cs_scaler" in input:
        cs_scaler = input["cs_scaler"]
        cs_scaler[cs_scaler == 0] = 1
        result["cs_scaler"] = torch.tensor(
            input["cs_scaler"], dtype=torch.float32, device=device
        )

    if "M" in input:
        result["M"] = torch.tensor(input["M"], dtype=torch.long, device=device)
    ...
    if "lattice" in input:
        result["lattice"] = torch.tensor(input["lattice"], device=device)
    if "D" in input:
        result["D"] = torch.tensor(input["D"], device=device)

    if "V" in input:
        result["V"] = torch.tensor(input["V"], device=device)

    if "NA" in input:
        result["NA"] = torch.tensor(input["NA"], dtype=torch.long, device=device)

    # rotation matrix
    result["RM"] = None
    if "RM" in input and input["RM"] is not None:
        result["RM"] = torch.tensor(
            input["RM"],
            # dtype=torch.float64,
            device=device,
        )
    ...
    if "Ai" in input:
        result["Ai"] = torch.tensor(input["Ai"], dtype=torch.float32, device=device)
    if "Pij" in input:
        result["Pij"] = torch.tensor(input["Pij"], dtype=torch.float32, device=device)
    return result


def extensive_train_loader(
    data,
    env_provider=None,
    batch_size=32,
    n_rotations=0,
    ...
    if isinstance(freeze_rotations, list):
        ...
        if freeze_rotations:
            ...
            thetas += [
                np.random.uniform(-np.pi, np.pi, size=3) for _ in range(n_rotations)
            ]
    ...
    while True:
        ...
        for r in range(n_rotations + 1):
            ...
            while (split + 1) * batch_size <= n_data:
                # Output a batch
                data_batch_idx = data_atom_indices[
                    split * batch_size : (split + 1) * batch_size
                ]
                data_batch = {
                    k: standardize_batch(data[k][data_batch_idx])
                    for k in data
                    if k != "labels"
                }
                if "labels" in data:
                    data_batch["labels"] = [
                        data["labels"][idx] for idx in data_batch_idx
                    ]
                # get neighbors
                if env_provider is not None:
                    if "lattice" in data_batch:
                        (
                            ...
                            distances,
                            distance_vectors,
                        ) = env_provider.get_environment(
                            data_batch["R"], data_batch["Z"], data_batch["lattice"]
                        )
                    else:
                        (
                            neighbors,
                ...
                if r > 0:
                    ...
                    if "F" in data:
                        rot_forces = rotate_molecule(
                            data_batch["F"], theta=theta
                        )  # B, A, 3
                else:
                    if not keep_original:
                        theta = np.random.uniform(-np.pi, np.pi, size=3)
                        rot_atoms = rotate_molecule(
                            data_batch["R"], theta=theta
                        )  # B, A, 3
                        if distance_vectors is not None:
                            rot_dist_vec = rotate_molecule(
                                distance_vectors, theta=theta
                            )
                        if "F" in data:
                            rot_forces = rotate_molecule(
                                data_batch["F"], theta=theta
                ...
                if distances is not None and distance_vectors is not None:
                    batch_dataset.update({"D": distances, "V": rot_dist_vec})
            ...
            if left_len != 0 and drop_last:
                ...
                data_batch = {
                    k: standardize_batch(data[k][left_idx])
                    for k in data
                    for k in data
                    if k != "labels"
                }
...
def extensive_loader_rotwise(
    ...
    # get neighbors
    if env_provider is not None:
        neighbors, neighbor_mask, atom_mask = env_provider.get_environment(
            data["R"], data["Z"]
        )
    ...
    while True:
        ...
        while (split + 1) * batch_size <= n_data:
            ...
            for theta in thetas:
                rot_atoms = rotate_molecule(
                    data["R"][data_batch_idx], theta=theta
                )  # B, A, 3
            ...
            rot_atoms = rot_atoms.reshape(
                rsize[0] * rsize[1], rsize[2], rsize[3]
            )  # B*n_rot, A, 3
            ...
            if env_provider is None:
                ...
                Z = np.tile(Z, (1, n_rotations + 1)).reshape(
                    Z.shape[0], n_rotations + 1, Z.shape[1]
                )  # B,n_rot,A
                ...
                N = np.tile(N, (1, n_rotations + 1, 1)).reshape(
                    N.shape[0] * (n_rotations + 1), N.shape[1], N.shape[2]
                )  # B*n_rot,A,A-1

                NM = neighbor_mask[data_batch_idx]  # B,A,A-1
                NM = np.tile(NM, (1, n_rotations + 1, 1)).reshape(
                    NM.shape[0] * (n_rotations + 1), NM.shape[1], NM.shape[2]
                )  # B*n_rot,A,A-1

                AM = atom_mask[data_batch_idx]  # B,A
                AM = np.tile(AM, (1, n_rotations + 1)).reshape(
                    AM.shape[0] * (n_rotations + 1), AM.shape[1]
                )  # B*n_rot,A
...
def extensive_voxel_loader(
    data, env_provider=None, batch_size=32, device=None, shuffle=True, drop_last=False
):
    r"""
    The main function to load and iterate data based on the extensive environment provider.
