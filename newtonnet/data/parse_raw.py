...
def split(data, train_size, test_size, val_size, random_states=90, stratify=None):
    ...
    tr_ind, val_ind = train_test_split(
        list(range(data["R"].shape[0])),
        test_size=val_size,
...
def h2_reaction(reaction_number, settings, all_sets):
    ...
    if settings["data"]["normal_mode"] and nm is not None:
        ...
        nm_select = sample_without_replacement(
            nm["R"].shape[0], n_nm, random_state=settings["data"]["random_states"]
        )
        if aimd is not None:
            for k in aimd.keys():
            ...
            data = None
            warnings.warn(
                "both AIMD and normal mode data for reaction# %i are missing."
                % reaction_number
            )
...
def parse_h2_reaction(settings, device):
    ...
    n_select = sample_without_replacement(
        dtrain["R"].shape[0], n_train, random_state=settings["data"]["random_states"]
    )
    ...
    print(
        "# data (train,val,test,irc): %i, %i, %i, %i"
        % (n_tr_data, n_val_data, n_test_data, n_irc_data)
    )
    ...
    if project not in ["voxel_cart_rotwise.py"]:
        ...
        irc_gen = extensive_train_loader(
            data=irc,
            env_provider=env,
            batch_size=val_batch_size,
            n_rotations=val_rotations,
        ...
        train_gen = extensive_loader_rotwise(
            data=dtrain,
            env_provider=env,
            batch_size=tr_batch_size,
            n_rotations=tr_rotations,
        ...
        val_gen = extensive_loader_rotwise(
            data=dval,
            env_provider=env,
            batch_size=val_batch_size,
            n_rotations=val_rotations,
        ...
        irc_gen = extensive_loader_rotwise(
            data=irc,
            env_provider=env,
            batch_size=val_batch_size,
            n_rotations=val_rotations,
        ...
        test_gen = extensive_loader_rotwise(
            data=dtest,
            env_provider=env,
            batch_size=val_batch_size,
            n_rotations=val_rotations,
...
def parse_nmr_data(settings, device, test_only=False):
    ...
    if test_only:
        ...
        for protein in train_proteins:
            ...
            for residue in data:
                ...
                shift_type_filter = np.array(
                    [item in shift_types for item in residue["Z"]]
                )
    ...
    if test_only:
        ...
        n_test_data = len(dtest["R"])
        print(
            "data size: (train,val,test): %i, %i, %i"
            % (n_tr_data, n_val_data, n_test_data)
        )
...
def parse_ani_data(settings, device):
    ...
    for train_data_num in train_data:
        ...
        for molecule in ani_data:
            ...
            if settings["data"]["subtract_self_energy"]:
                self_energy = np.sum(
                    [atomic_self_energy[a] for a in molecule["species"]]
                )
            ...
            dtrain["Z"].extend(
                np.tile(
                    [atomic_Z_map[a] for a in molecule["species"]], (n_conf_train, 1)
                )
            )
            ...
            if test_data is False:
                ...
                dtest["Z"].extend(
                    np.tile(
                        [atomic_Z_map[a] for a in molecule["species"]], (n_conf_test, 1)
                    )
                )
    ...
    if test_data:
        for test_data_num in test_data:
            ...
            for molecule in ani_data:
                ...
                dtest["Z"].extend(
                    np.tile([atomic_Z_map[a] for a in molecule["species"]], (n_conf, 1))
                )
    ...
    if total_proportion < 1:
        train_val_indices, unused_indices = train_test_split(
            further_split_indices,
            train_size=total_proportion,
            random_state=settings["data"]["train_val_split_random_state"],
        )
    else:
        train_val_indices = further_split_indices
    train_idx, val_idx = train_test_split(
        train_val_indices,
        test_size=(val_proportion / total_proportion),
        random_state=settings["data"]["train_val_split_random_state"],
    )
    data = dtrain
    dtrain = {}
...
def parse_methane_data(settings, device):
    ...
    for composition in all_data:
        ...
        n_atoms = len(Z)
        coords = np.load(
            os.path.join(train_path, composition, "set.000", "coord.npy")
        ).reshape((-1, n_atoms, 3))
        energy = np.load(os.path.join(train_path, composition, "set.000", "energy.npy"))
        force = np.load(
            os.path.join(train_path, composition, "set.000", "force.npy")
        ).reshape((-1, n_atoms, 3))
    ...
    further_split_indices, test_idx = train_test_split(
        all_indices,
        test_size=settings["data"]["test_count"],
        random_state=settings["data"]["train_test_split_random_state"],
    )
