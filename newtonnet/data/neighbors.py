...
class ExtensiveEnvironment(object):
    ...
    def get_environment(self, positions, atomic_numbers):
        ...
        neighbors = N[~np.eye(n_atoms, dtype=bool)].reshape(n_atoms, -1)  # (A, A-1)
        ...
        if n_atoms < self.max_n_neighbors:
            neighbors = padaxis(
                neighbors, self.max_n_neighbors, axis=-1, pad_value=-1
            )  # (D, A, N)
            atomic_numbers = padaxis(
                atomic_numbers, self.max_n_neighbors, axis=-1, pad_value=0
            )  # (D, A, N)
...
class PeriodicEnvironment(object):
    ...
    def get_environment(self, positions, atomic_numbers, lattice):
        ...
        staggered_distance_vectors = [
            [[] for _ in range(n_atoms)] for _ in range(n_data)
        ]
        ...
        for idx_data in range(n_data):
            ...
            ase_molecule = Atoms(
                molecule_Zs, positions=molecule_Rs, cell=molecule_lattice, pbc=True
            )
            for i, j, dist, vec in zip(
                *neighbor_list("ijdD", ase_molecule, self.cutoff)
            ):
        ...
        for i in range(n_data):
            for j in range(n_atoms):
                if neighbor_count[i, j] > 0:
                    ...
                    distance_vectors[
                        i, j, : neighbor_count[i, j]
                    ] = staggered_distance_vectors[i][j]
