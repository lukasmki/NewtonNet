...
def rotate_molecule(atoms, theta=None):
    ...
    # handle theta
    if theta is None:
        theta = np.random.uniform(-np.pi / 2.0, np.pi / 2.0, size=3)
...
def padaxis(array, new_size, axis, pad_value=0, pad_right=True):
    ...
    assert (
        add_size >= 0
    ), "Cannot pad dimension {0} of size {1} to smaller size {2}".format(
...
def standardize_batch(batch_content, return_mask=False):
    ...
    if type(batch_content[0]) is np.ndarray:
        ...
        batch_content = np.array(
            [padaxis(arr, maximal_len, 0) for arr in batch_content]
        )
    else:
...
def check_data_consistency(data):
    ...
    if sorted(data.keys()) != keys and sorted(data.keys()) != sorted(
        ["CE", "CF", "E", "F", "N", "R", "RXN", "Z"]
    ):
        raise ValueError(
            f"Data keys {sorted(data.keys())} does not match expected {keys} key!"
        )
    ...
    if data["R"].shape != (n_data, n_atom, 3):
        raise ValueError(
            f"The data['R'] shape {data['R'].shape} != {(n_data, n_atom, 3)}"
        )
    ...
    if data["F"].shape != (n_data, n_atom, 3):
        raise ValueError(
            f"The data['F'] shape {data['F'].shape} != {(n_data, n_atom, 3)}"
        )
...
def combine_rxn_arrays(rxns, n_max=6):
    ...
    for index, rxn in enumerate(rxns):
        ...
        if sorted(rxn.keys()) != sorted(data.keys()):
            raise ValueError(
                f"RXN {index} keys={sorted(rxn.keys())} != {sorted(data.keys())}"
            )
        ...
        if n_atom > n_max:
            raise ValueError(
                f"RXN {index} has {n_atom} atoms which is greater than n_max={n_max}"
            )
...
def get_data_subset(data, rxn_num):
    ...
    if rxn_num not in rxn_nums:
        raise ValueError(
            f"Reaction {rxn_num} does not exist among existing {rxn_nums} rxns!"
        )
...
def get_data_remove_appended_zero(data):
    ...
    if len(rxn_nums) != 1:
        if (
            not len(rxn_nums) == 2 and rxn_nums[0][:-1] == rxn_nums[1][:-1]
        ):  # 2 spin state reaction
            raise ValueError(
                f"Cannot remove appended zero as {rxn_nums} rxns were found!"
            )
    ...
    if len(n_atom) != 1:
        raise ValueError(
            f"Cannot remove appended zero as {n_atom} number of atoms were found!"
        )
    n_atom = n_atom[0]
    ...
    if len(nums) != 1:
        raise ValueError(
            f"Cannot remove appended zero as {nums} atomic numbers were found!"
        )
    if np.count_nonzero(nums[0] != 0) != n_atom:
        raise ValueError(
            f"Cannot remove appended zero as number of non-zero atomic numbers does "
